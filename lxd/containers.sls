...
lxd_container_{{ remotename }}_{{ name }}_touch_executed:
    ...
    {%- else %}
        - m_name: "{{ name }}"
    {%- endif %}
        - cmd: [ 'touch', '/etc/_salt_lxd_bootstrapped']
        - remote_addr: "{{ remote.remote_addr }}"
        - cert: "{{ remote.cert }}"
        - key: "{{ remote.key }}"
        - verify_cert: {{ remote.verify_cert }}
...
lxd_container_{{ remotename }}_{{ name }}_check_executed:
    ...
    {%- else %}
        - m_name: "{{ name }}"
    {%- endif %}
        - src: "/etc/_salt_lxd_bootstrapped"
        - dst: "/tmp/_salt_lxd_bootstrapped_{{ remotename }}_{{ name }}"
        - uid: 0
        - gid: 0
        - overwrite: True
        - remote_addr: "{{ remote.remote_addr }}"
...
lxd_container_{{ remotename }}_{{ name }}_bsc_{{ loop.index }}:
    ...
    {%- endif %}
        ...
        - unless:
            - "grep -s -q True /tmp/_salt_lxd_bootstrapped_{{ remotename }}_{{ name }}"
      {%- endif %}
...
lxd_container_{{ remotename }}_{{ name }}_make_executed:
    ...
    {%- else %}
        - m_name: "{{ name }}"
    {%- endif %}
        - src: "salt://lxd/files/_salt_lxd_bootstrapped"
        - dst: "/etc/_salt_lxd_bootstrapped"
        - mode: 0600
        - remote_addr: "{{ remote.remote_addr }}"
        - cert: "{{ remote.cert }}"
        - key: "{{ remote.key }}"
